...
class FaceAging(object):
    ...
    def face_age_alexnet(self, x, scope_name='alexnet', if_age=False, reuse=False):
        with tf.variable_scope(scope_name, reuse=reuse) as scope: #큰 변수 세트를 공유할 필요가 있고 (한 곳에서만 모든 변수의 초기화; 일종의 flag 외 글로벌 변수 )-namespace 관리
            # 1st Layer: Conv (w ReLu) -> Pool -> Lrn
            conv1 = conv(x, 11, 11, 96, 4, 4, padding='VALID', name='conv1')
            pool1 = max_pool(conv1, 3, 3, 2, 2, padding='VALID', name='pool1')
            norm1 = lrn(pool1, 2, 2e-05, 0.75, name='norm1')
            ...
            if if_age:
                ...
                self.age_logits = fc(age_dropout7, 4096, 5, name='age_fc8', relu=False) #5->2
            print("age_logits",self.age_logits)
            return scope
    ...
    def PatchDiscriminator(self, image, name, condition=None, mode='train', reuse=False):
        #. D(x) 구해야 하는 데 x가 image(_,128,128,3)....=실제데이터
        #  실제 데이터 전체(patch) & label 의 loss !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! ??????????????????????????????????????
        ...
        with tf.variable_scope(name):
            ...
            #실제 구현시에는 FCN의 Idea를 적용해서 Fully Convolutional로 마지막을 구성하게 되면 Single 'Real/Fake Score'가 아닌 'Real/Fake Score' Map이 나오게 된다.
            # 이것을 이와 동일한 Size의 Label (ex. Real이면 1, Fake면 0)과의 Loss를 정의해서 Optimize를 하면 된다.
            print(x.get_shape()) #(1,1,1)

            return x

    # conditional lsgan + batchnorm
    #true_label_fea_128= condition
...
###########condition=None
        print("generating..")
        self.g_source = self.ResnetGenerator(source_img_128, name='generator', condition=None) #G(x)

        discriminator = self.PatchDiscriminator

        # real image, right label
        D1_logits = discriminator(imgs, name='discriminator', condition=None) #D(x)
        # real image, false label
        #D2_logits = discriminator(imgs, name='discriminator', condition=None, reuse=True)#D(x)
        # fake image, true label
        D3_logits = discriminator(self.g_source, name='discriminator', condition=None, reuse=True)#D(G(x))
    ...
    #---------------------------위는 loss, 아래는 optimizing------------------------------------------------
        ...
        # var_list
        # d_optim= An Operation that updates the variables in var_list. global_step = global_step+1

        train_ops = [d_optim] + self._extra_train_ops  #_extra_train_ops: batch norm gamma beta 업데이트 연산

        self.d_optim = tf.group(*train_ops) #tf.group: Create an op that groups multiple operations.
        print("g_optim..")
    ...
    def generate_images(self, source_img_128, true_label_fea, stable_bn=False, reuse=False, mode='test'):
        ...
        g_source = self.ResnetGenerator(source_img_128, name='generator', condition=None,
