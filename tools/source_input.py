...
#"filename label"저장 파일인 ("source_file")이용하여 resnet Generator 입력 생성
#data_generator 와 달리 라벨 별 배치 x
# source_img_227, source_img_128, face_label= image_227_batch, image_128_batch, label_batch
#**특징: 배치가 한 번 생성 !!!!!!!!!???????????= Generator에 들어가는 이미지수는 one batch size =32 ?????????????
...
def read_images3(input_queue):
    ...
    image = tf.reshape(image, [IM_HEIGHT, IM_WIDTH, IM_CHANNELS]) # 400*400*3
...
# age_lsgan_transfer 에서 호출----------------------------------------------------------------------------
def load_source_batch3(filename, img_folder, batch_size, shuffle=True):

    img_list, label_list = get_imgAndlabel_list2(filename, img_folder) # file name : "filename label"저장 파일
    ...
    image_227_batch, image_128_batch, label_batch = tf.train.shuffle_batch(
        ...
        capacity=1280,
        min_after_dequeue=640)
    print('exit load_source_batch3 ; batch has source images ', label_batch)
...
def get_imgAndlabel_list2(filename, img_folder):
    ...
    label list
    """
    f = open(filename, 'r',encoding='utf-8-sig')#encoding='utf8'
    lines = f.readlines()
    print(len(lines))
    f.close()
    imgname_lists = []
