...
flags.DEFINE_integer("age_groups", 2, "the number of different age groups")

flags.DEFINE_integer('max_steps', 500000, 'Number of batches to run')

flags.DEFINE_string("alexnet_pretrained_model", "./checkpoints/pretrained/alexnet.model-292000",
                    "Directory name to save the checkpoints")

flags.DEFINE_string("age_pretrained_model", "./checkpoints/pretrained/age_classifier.model-300000",
                    "Directory name to save the checkpoints")
...
flags.DEFINE_string("source_file", './images/source.txt', "source file path")

flags.DEFINE_string("root_folder", "./images/cropped/", "folder that contains images")
...
def my_train():
    with tf.Graph().as_default():
        ...
        imgs = tf.placeholder(tf.float32, [FLAGS.batch_size, FLAGS.image_size, FLAGS.image_size, 3])        # (_,128,128,3)
 ...
 #           print(face_label.eval())
       ...
       # age_label 이아니라 face_label 사용해야 하는 거 아닌가 ?????????????
        ...
        for step in range(FLAGS.max_steps): #200000
            ...
            dict = {imgs: images,
                    true_label_features_128: t_label_features_128,#condition for resnet generator
                    true_label_features_64: t_label_features_64,#condition for discriminator
                    false_label_features_64: f_label_features_64,#condition for discriminator
  ...
  #          import ipdb
   #         ipdb.set_trace()
            for i in range(d_iter):# 1
                _, d_loss = sess.run([model.d_optim, d_error], feed_dict=dict)  # dict가 위 placeholder 에 모두 feed?
